...
const sections: Section[] = [
  ...
  },
  {
    ...
    items: [
      ...
      'If they have met the Round Goal in the table below, they can optionally lay down the eligible sets/runs in front of them face up. They can only "lay down" once per round, and it must be the same amount of sets/runs as specified in the Round Goal. For example, if they are on Round 2, they can lay down a total of 1 Run and 1 Set. They cannot lay down 2 sets, even if they have 2 sets.',
      'If they have already laid down their sets/runs, they can optionally place other cards from their hand on their own sets/runs or their opponents\'s sets/runs. For example, if they draw a card that plays at the beginning or end of an opponent\s run, they can keep it concealed in their hand and play it during their turn in any round.',
  ...
  },
  {
    ...
    body:'When a player discards a card at the end of their turn, that card becomes available for the next player to take on their turn. If they decide to take the discard, it serves as their draw for that turn. If they decide not to take it, the card is up for grabs for another player to steal. All remaining players other than the player who discarded will have a chance to steal the discard, but only if the player before them (going around the table in order) passes on it. When a card is stolen, the next card in the discard pile does not become available to steal. The next available steal will be the current player\'s discard. When a player steals a discard, it does not count as their upcoming draw. It is an extra card in their deck. There is no limit for how many cards a player can steal. But remember, points are bad! If a player steals too many cards, they might end up in big trouble at the end of the round.',
  },
  {
    heading: 'Sets and Runs',
    body:'A set is 3 or more cards of the same number. A run is 4 or more cards of the same suit in sequential order. For runs, the lowest card (1) can also be the highest card. For example, a run can be 12, 13, 14, 1. Only the 1 card can be used at either end. For example, a run cannot it cannot be 13, 14, 1, 2. The Rook card is a wild card and can be used as a substitute for any card when forming sets or runs. For example, if you have a 3 of Red, a 4 of Red, a 6 of red, and the Rook card, you can use the Rook as a 5 of Red to form a run. Or if you have two 7s and the Rook, you can use the Rook as a third 7 to form a set.',
  },
  {
